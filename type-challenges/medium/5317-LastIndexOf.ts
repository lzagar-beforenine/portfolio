...
type LastIndexOf<
  ...
  TLastIndex extends number[] | never = never
> = TArray extends [infer First, ...infer Rest]
  ? IsEqual<TValue, First> extends true
    ? LastIndexOf<Rest, TValue, [...TIndex, 0], TIndex>
    : LastIndexOf<Rest, TValue, [...TIndex, 0], TLastIndex>
  : [TLastIndex] extends [never]
  ? -1
  : TLastIndex["length"];
