...
> = TArray["length"] extends High
  ? [...TArray, High]
  : CounterArray<High, [...TArray, TArray["length"]]>;

type NumberRange<
  Low extends number,
  High extends number,
  TArray extends number[] = CounterArray<High>
> = TArray extends [infer First, ...infer Rest extends number[]]
  ? First extends Low
    ? TArray[number]
    : NumberRange<Low, High, Rest>
  : never;
