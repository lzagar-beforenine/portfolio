...
type Exist<TArray, TValue> = TArray extends [infer First, ...infer Rest]
  ? IsEqual<First, TValue> extends true
    ? true
    : Exist<Rest, TValue>
  : -1;

type Unique<TArray, TResult extends unknown[] = []> = TArray extends [
  infer F,
  ...infer R
]
  ? Exist<TResult, F> extends -1
    ? Unique<R, [...TResult, F]>
    : Unique<R, [...TResult]>
  : TResult;
