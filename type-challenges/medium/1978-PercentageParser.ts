...
> = TString extends `${infer First}${infer Rest}`
  ? First extends ValType
    ? PercentageParser<Rest, First, Value, Type>
    : First extends PercentType
    ? PercentageParser<Rest, Type, Value, First>
    : PercentageParser<Rest, Type, `${Value}${First}`, Percent>
  : [Type, Value, Percent];
